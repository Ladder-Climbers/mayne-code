...
const encry2arr_from = function (t, e, r) {  // 1  39  50  66
...
function decrypt(r) {
    ...
    // return JSON.stringify(data['payload']['items'])
...
const search = async (key, page = 0) => {
    ...
    const index = text.search(patten);
    if (index < 0) return null;
    let data = text.slice(index + "window.__DATA__ = \"".length);
    data = data.slice(0, data.search('\n'));
